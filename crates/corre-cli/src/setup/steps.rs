...
pub fn review_and_write(state: &mut SetupState, term: &console::Term) -> anyhow::Result<()> {
    ...
    if let Some(ref topics) = state.topics_yml {
        let config_subdir = data_dir.join("config");
        std::fs::create_dir_all(&config_subdir)?;
        std::fs::write(config_subdir.join("topics.yml"), topics)?;
        println!("  Wrote {}/config/topics.yml", data_dir.display());
    }
...
fn print_summary(state: &SetupState) {
    ...
    println!("  Config:     {data_dir}/corre.toml");
    println!("  Topics:     {data_dir}/config/topics.yml");

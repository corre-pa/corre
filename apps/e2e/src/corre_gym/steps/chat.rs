...
async fn asks_for_status(world: &mut GymWorld, alias: String) {
    let reply = http::send_chat(world, &alias, "What's the status of my current workout?")
        .await
...
async fn starts_session(world: &mut GymWorld, alias: String) {
    let reply = http::send_chat(
        world,
        &alias,
        "I'm at the gym now and starting my workout. Please open a new session for me.",
    )
    .await
...
async fn ends_session(world: &mut GymWorld, alias: String) {
    let reply = http::send_chat(world, &alias, "I'm done — please end this workout session.")
        .await
        .expect("end-session message failed");
    world.last_reply = Some(reply);
    world.current_user = Some(alias);
}

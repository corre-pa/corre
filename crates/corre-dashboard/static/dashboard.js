...
(function () {
        ...
        var AUTHENTICATED = !!window.__AUTHENTICATED;
        ...
        var startSettingsEl = document.getElementById("start-settings");
        if (startSettingsEl) {
            startSettingsEl.addEventListener("click", function (e) {
                e.preventDefault();
        ...
        // Start > MCP Store
        var startMcpStoreEl = document.getElementById("start-mcp-store");
        if (startMcpStoreEl) {
            startMcpStoreEl.addEventListener("click", function (e) {
                e.preventDefault();
        ...
        // =========================================================================
        // Authenticated-only UI: Settings, Store, Config Editor, Modals
        // =========================================================================
        if (AUTHENTICATED) {
            ...
            function gatherSettingsForm() {
                ...
                document.querySelectorAll("#appsConfig .xp-dynamic-row").forEach(function (row) {
                    var cap = {
                        ...
                        mcp_servers: row.querySelector(".cap-cfg-mcp-servers").value.trim().split(",").map(function (s) {
                            return s.trim();
                        }).filter(Boolean),
            ...
            function addAppRow(cap) {
                ...
                row.querySelector(".xp-button-remove").addEventListener("click", function () {
                    row.remove();
                });
            ...
            function cfgBuildListField(field, items) {
                ...
                items.forEach(function (item) {
                    cfgAddListItem(itemsContainer, field.fields || [], item);
                });
                ...
                addBtn.addEventListener("click", function () {
                    cfgAddListItem(itemsContainer, field.fields || [], {});
                });
            ...
            function cfgGatherField(el, field) {
                ...
                if (field.type === "text-list") {
                    ...
                    return raw ? raw.split(",").map(function (s) {
                        return s.trim();
                    }).filter(Boolean) : [];
                }
            ...
            function cfgLoad(capName, schema) {
                ...
                xhr.onload = function () {
                    ...
                    if (xhr.status === 200 && xhr.responseText.trim()) {
                        try {
                            data = jsyaml.load(xhr.responseText);
                        } catch (e) {
                            data = {};
                        }
                    }
            ...
            function renderStoreTable() {
                ...
                storeTbody.querySelectorAll(".store-install-btn").forEach(function (btn) {
                    btn.addEventListener("click", function () {
                        ...
                        var entry = storeCatalog.servers.find(function (s) {
                            return s.id === id;
                        });
            ...
            function renderAppStoreTable() {
                ...
                var installedNames = {};
                installedApps.forEach(function (c) {
            ...
            function triggerRestart() {
                ...
                setTimeout(function poll() {
                    fetch("/api/dashboard/status?token=" + encodeURIComponent(TOKEN))
                        .then(function (r) {
                            if (r.ok) location.reload(); else setTimeout(poll, 1000);
                        })
                        .catch(function () {
            ...
            document.getElementById("configure-modal-save").addEventListener("click", function () {
                ...
                var args = argsStr ? argsStr.split(",").map(function (s) {
                    return s.trim();
                }) : [];
                var env = {};

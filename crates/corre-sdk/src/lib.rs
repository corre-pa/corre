//! SDK for writing Corre app plugins.
//!
//! An app plugin is a standalone binary that communicates with the Corre host over
//! stdin/stdout using CCPP v1 (Corre App Plugin Protocol), a JSON-RPC 2.0 protocol.
//! This crate provides everything a plugin needs: the [`AppClient`] async helper,
...
//!
//! ```rust,ignore
//! use corre_sdk::{AppClient, LlmRequest};
...
//!         &params.app_name, params.log_dir.as_deref(), params.log_level.as_deref(),
//!     );
//!
...
//! | Module | Description |
//! |--------|-------------|
//! | [`client`] | [`AppClient`] — async CCPP client with background reader, request multiplexing, and typed RPC methods for MCP tools, LLM completions, file output, and progress reporting |
...
//! | [`types`] | Core output types: [`AppOutput`](types::AppOutput), [`Section`](types::Section), [`Article`](types::Article), [`Source`](types::Source), [`ContentType`](types::ContentType), [`CustomContent`](types::CustomContent) |
//!
//! # CCPP error codes
//!
...
pub use types::{AppManifest, AppOutput, Article, ContentType, CustomContent, Section, Source};

use tracing_subscriber::{Layer, layer::SubscriberExt, util::SubscriberInitExt};

/// Set up tracing for an app plugin.
///
...
///   `{log_dir}/{app_name}-yyyy-mm-dd.log`
///
...
pub fn init_tracing(app_name: &str, log_dir: Option<&str>, log_level: Option<&str>) -> Option<tracing_appender::non_blocking::WorkerGuard> {
    ...
    let (file_layer, guard) = match log_dir {
        Some(dir) => {
            let appender = tracing_appender::rolling::daily(dir, app_name);

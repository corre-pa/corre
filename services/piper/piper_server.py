...
class Handler(http.server.BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        speaker = str(body.get("speaker", DEFAULT_SPEAKER)).strip()
        if speaker and speaker not in VALID_SPEAKERS:
            self.send_error(
                400,
                f"unknown speaker '{speaker}', valid: {', '.join(sorted(SPEAKER_ID_MAP.keys()))}",
            )
            return

        # Resolve name -> numeric ID (piper CLI only accepts numeric IDs)
        if speaker and speaker in SPEAKER_ID_MAP:
            speaker = str(SPEAKER_ID_MAP[speaker])
